# Home Craft Guilds v0.1.160 — повна локалізація та виправлення preview

- Доперекладено пропущені елементи складу гільдії, талантів, досягнень, екрана створення гільдії, підписів ліжок, tooltip-ів предметів і NPC admin UI.
- Додано переклади для портальних досягнень, щоб англійський інтерфейс не показував українські fallback-тексти.
- Всі кастомні назви зброї, броні, інструментів і елітних товарів тепер мають `trade_name.homecraftguild.*` ключі.
- Ванільна їжа та метали більше не отримують кастомні назви: вони використовують оригінальний переклад Minecraft і реагують на зміну мови.
- Додано перевірку parity для placeholder-ів `%s`/`%d` між `en_us` та `uk_ua`.
- 3D preview NPC вирівняний по центру preview-вікна й далі реагує на мишку.
